...
use super::{
    ...
    resources::*,
    rules::GoBoardRules,
    utils::{RenderUtils, CoordinateUtils},
};
...
pub fn handle_place_stone(
    ...
    for event in stone_events.read() {
        ...
        if config.0.enable_captures || config.0.enable_ko_rule {
            ...
            // 简单检查
            if x < 0 || x >= config.0.board_size.get_value() || y < 0 || y >= config.0.board_size.get_value() {
                continue;
            }
...
fn draw_board(
    ...
    let metrics = RenderUtils::calculate_board_metrics(window_size, config.board_size, config.adaptive_padding);
    ...
    if config.show_coordinates {
        draw_coordinates(commands, board_size_value, metrics.cell_size, metrics.half_board, config.coordinate_color);
    }
}
...
fn draw_single_stone(
    ...
    // 计算棋子位置
    let world_pos = CoordinateUtils::board_to_world(position, config.board_size, window_size, config.adaptive_padding);
    ...
    commands.spawn((
        ...
        Transform::from_translation(Vec3::new(world_pos.x + shadow_offset, world_pos.y - shadow_offset, 3.8)),
        StoneShadow,
    ));
